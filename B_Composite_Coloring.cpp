...
void sieve(int ub) { 
    ...
    REP(i, 2, sz+1) 
        if (bs[i]){
        for (int j = i * i; j <= sz; j += i)sF[j] = i, bs[j] = 0;
        sF[i] = i; 
    }
} 
...
void solve(){
    int n; cin >> n;
    map<int, vi> mp;
    int arr[n]; REP(i, 0, n) cin >> arr[i], mp[sF[arr[i]]].pb(arr[i]);

    map<int, int> mp2;
    int i = 1;
    for(auto p : mp){
        for(int j : p.S)mp2[j] = i;
        i++;
    }

    println(i-1);

    REP(i, 0, n) prints(mp2[arr[i]]);
    ...
    // REP(i, 0, n) cout << sF[arr[i]] << " \n"[i == n-1];

    // REP(i, 0, 30) prints(sF[i]);
