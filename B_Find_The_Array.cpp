...
bool check(int* arr, int *a2, int n){
    int val = 0;
    REP(i, 0, n) val += abs(arr[i]-a2[i]);
    if(2*val < sum(arr, n)){
        REP(i, 0, n) prints(a2[i]);
        println("");
        return true;
    }
    return false;
...
void solve(){
    int n; cin >> n;
    int arr[n], odd[n], even[n]; REP(x, 0, n){
        cin >> arr[x];

        if(x&1)odd[x] = arr[x], even[x] = 1;
        else odd[x] = 1, even[x] = arr[x];
    }

    if(check(arr, odd, n)) return;
    check(arr, even, n);
    // int avg = average(arr, n), tst = 0;
    // int sm = sum(arr, n);
    // REP(i, 0, n) tst += abs(avg-arr[i]);
    // int tst1 = abs(avg-sm);
    
    // bug(avg, tst, tst1, 2*tst, sum(arr, n), 2*tst < sum(arr, n));
    // REP(i, 0, n) cout << 1 << " \n"[i == n-1];
}
