...
int binaryExponentiation(int a, int b, int mod = M1){
    // returns a^b;
    int res = 1;
    while(b){
        if(b&1) res = (res * a)%mod; /* considering mod equals 1e9+7 if not change accordingly*/
        b >>= 1;
        a = (a * a)%mod;
    }
    return res;
...
void solve(){
    ...
    bool f = false;
    int ans;
    if(n%3 == 0)ans = binaryExponentiation(3, n/3); 
    else if(n%3 == 2){
        ans = binaryExponentiation(3, (n-2)/3);
        ans = (ans * 2)%M1; 
    }else{
        ans = binaryExponentiation(3, (n-4)/3);
        ans = (ans*4)%M1;
    }
