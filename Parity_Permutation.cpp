...
int fact(int n){
    int res = 1;
    REP(i, 1, n+1) res *= i, res %= M1;
    return res;
}
void solve(){
    int n, k, p = 0, q = 0, ans = 0; cin >> n >> k;
    int a[n]; REP(x, 0, n) cin >> a[x], a[x]&1 ? p++ : q++;
    if(k == 0){
        if(p != 0 && q != 0) return println(ans);
        return println(fact(n)); 
    }

    int evenPos = n/2, oddPos = n-evenPos;
    if((evenPos == p && oddPos == q) || (oddPos == p && evenPos == q)){
        ans = fact(p);
        ans *= fact(q);
        ans %= M1;
    }
    if((oddPos == p && evenPos == q) && (evenPos == p && oddPos == q)){
        ans *= 2;
        ans %= M1;
    }
