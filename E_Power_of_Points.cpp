...
void solve(){
    int n, sm = 0; cin >> n;
    int arr[n], cpy[n]; REP(x, 0, n) cin >> arr[x];
    int pre[n+1], suf[n+1];
    pre[0] = 0, suf[n] = 0;
    REP(i, 1, n+1) pre[i] = arr[i-1]+pre[i-1];
    for(int j = n-1; j >= 0; --j) suf[j] = suf[j+1]+arr[j];
    // _print(arr, n);
    // _print(pre, n+1);
    // _print(suf, n+1);
    REP(i, 0, n){
        int cur2 =  pre[i]+ suf[i] - (n-i+1)*arr[i] + n;
        int cur = n;
        REP(j, 0, n) cur += abs(arr[i]-arr[j]);
        cout << cur2 << " \n"[i==n-1]; 
    }
}
