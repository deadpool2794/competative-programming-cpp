...
int moves(char _a, char _b){
    int a = _a-'0', b = _b-'0';
    int inc = 0, dec = 0;
    int tempa = a, tempb = b;
    while(tempa != tempb)tempa++, inc++, tempa %= 10;
    while(a != b)a--, dec++, a += 10, a%= 10;
...
void solve(){
    int n, m; cin >> n >> m;
    string lock, key; cin >> lock >> key;
    int ans = 1e9;
    REP(i, m, n+1){
        int cur = 0;
        for(int j = i-m, k = 0; j < i; ++j, ++k){
            if(lock[j] != key[k])cur += moves(lock[j], key[k]);
        }
        ans = min(ans, cur);
    }
    println(ans);
