...
struct Matrix{
    ...
    Matrix(bool iden){
        // if iden is true -> initializes identity matrix else matrix with all 0s
        memset(mat, 0, sizeof(mat));
        mod = M1; // chnage if mod is not 1e9+7
        if(iden) REP(i, 0, sz) mat[i][i] = 1;
    }
    ...
    Matrix operator* (Matrix a){
        ...
        REP(i, 0, sz) REP(j, 0, sz) REP(k, 0, sz)
            res.mat[i][j] += mat[i][k]*a.mat[k][j], res.mat[i][j] %= mod;
        return res;
    }

    void print(){
        REP(i, 0, sz) REP(j, 0, sz) cout << mat[i][j] << " \n"[j == sz-1];
    }
};
...
int matrixExponentiation(int n){
    ...
    Matrix res(1), t(0);
    t.mat[0][0] = t.mat[0][1] = t.mat[1][0] = 1; // define T matrix
    while(n){
        if(n&1) res = res * t;
        t = t * t;
        n >>= 1;
    }
    return (res.mat[0][0]+res.mat[0][1])%M1; // chnage if mod is not 1e9+7 
}
...
void solve(){
    ...
    println(res.mat[0][0]+res.mat[0][1]);
