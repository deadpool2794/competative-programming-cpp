...
void dfs(int root, vvi&graph, vi& child){
    ...
    for(auto i : graph[root]) dfs(i, graph, child);

    child[root] = graph[root].size();

    for(auto i : graph[root]) child[root] += child[i];
}

void dfs2(int root, int lev, vi&parent, vvi& graph){
    parent[root] = lev;
    for(int i : graph[root]) dfs2(i, lev+1, parent, graph);
}

void solve(){
    int n, u, v; cin >> n;
    vvi graph(n);
    REP(i, 0, n-1) cin >> u >> v, u--, v--, graph[u].pb(v);
    vi child(n), vis(n, 0), parent(n, 0);
    REP(i, 0, n){
        if(!vis[i])vis[i] = 1, dfs(i, graph, child);
    }

    _print(child);
    int root = max_element(ALL(child)) - child.begin();
    dfs2(root, 0, parent, graph);
    _print(parent);
