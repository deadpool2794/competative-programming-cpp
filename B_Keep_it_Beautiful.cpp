...
void solve(){
    ...
    string ans;
    REP(x, 0, n) cin >> arr[x];
    int last = arr[0], first = arr[0];
    p1() ;
    int i = 1;
    while(i < n && arr[i] >= last) p1(), last = arr[i], i++;
    while(i < n ){
        if(arr[i] <= first){
            p1(), last = arr[i], i++;
            break;
        }
        else if (arr[i] >= last) p1(), last = arr[i], i++;
        else p0(), i++;

    }
    while(i < n){
        if(arr[i] >= last && arr[i] <= first) p1(), last = arr[i];
        else p0();
        i++;
    }
    println(ans);
