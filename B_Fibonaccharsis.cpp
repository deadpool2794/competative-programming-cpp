...
void print(int x, int y, int k){
    ...
    prints(x);
    prints(y);
    REP(i, 2, k)fib.pb(fib[i-1]+fib[i-2]), cout << fib[i] << " \n"[i==k-1]; 
}
...
void solve(){
    ...
    int ans = 0, cnt =0;
    for(int x = 0; x < 1e5+10; ++x){
        int a = coef[k-1].F, b = coef[k-1].S;
        if((n-(x*a))%b == 0){
            int y = (n-(x*a))/b;
            if(x <= y) ans++; 
            else cnt++;
...
int32_t main()
{
    ...
    coef.pb({1, 0});
    coef.pb({0, 1});
    REP(i, 0, 30)coef.pb({coef[i].F + coef[i+1].F, coef[i].S + coef[i+1].S});
    // _print(coef);
    int _ = 1;
    cin >> _;
    while (_--) solve();
