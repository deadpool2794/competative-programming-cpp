...
void solve(){
    ...
    int n, m, h;
    cin >> n >> m >> h;
    int cars[n]; REP(x, 0, n) cin >> cars[x];
    int watts[m]; REP(x, 0, m) cin >> watts[x];

    sort(cars, cars+n, [](int x, int y) {return x > y;});
    sort(watts, watts+m, [](int x, int y) {return x > y;});
    int ans = 0, i = 0;
    while(i < min(m, n)){
        ans += min(watts[i]*h, cars[i]);
        i++;
    }
