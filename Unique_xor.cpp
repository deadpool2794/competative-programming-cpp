...
pii make(string bin, int num){
    sort(ALL(bin));
    int one = 0, zer = 0;
    int c1, c0;
    
    c0 = c1 = 0;
    REP(i, 0, bin.size()) c0 +=  (bin[i] == '0'), c1 += (bin[i] == '1');
    if(c1 == 0) return {INT_MAX, 0};
    if(c0%2 == 1 && c1 % 2 == 0) return {c0, (c1)/2 +2};
    if(c0%2 == 0 && c1 %2 == 0) return {c0, (c1)/2};
    if(c0%2 && c1%2) return {c0, (c1)/2+2};
    return {c0, c1/2+2};
...
void solve(){
    ...
    REP(i, 0, k){
        int temp = i;
        string remArray;
        while(temp < n) remArray.pb(bin[temp]), temp += k;
        pii res = make(remArray, 0);
        one += res.F, zer += res.S;
    }
