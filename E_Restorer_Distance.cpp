...
const int N = 1e5+10;
int n, a, r, m, hi[N], l, h; 

int f(int mid){
    int minus = 0, plus = 0, cost = 0;
    REP(i, 0, n) mid > hi[i] ? plus += mid - hi[i] : minus += hi[i] - mid;
    int mn = min(minus, plus);
    if(mn * a + mn * r > mn * m ){
        cost += mn*m;
        cost += (minus - mn)* r;
        cost += (plus - mn)* a;
...
void solve(){
    ...
    if(first < second && second < third) println(first);
    else if (second < third && second < first) println(second);
    else println(third);
