...
void sieve(int ub) { 
    // Generates primes till upperbound ub(inclusive)
    bs.set(); // bitset bs -> to mark all composites -> initialized globally with max size needed
    bs[0] = bs[1] = 0; 
    REP(i, 2, ub+1) 
        if (bs[i]){
        for (int j = i * i; j <= ub; j += i) bs[j] = 0;
        primes.pb(i); // vi primes -> to store all prime numbes -> initialized globally
    }
} 
