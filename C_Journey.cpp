...
#define UCHIHA cout << boolalpha << fixed << setprecision(15);
...
void dfs(int cur, int par, ld prob, int pl){
    // bug(cur, graph[cur].size());
    // _print(graph[cur]);
    if(graph[cur].size() == 1){
        // bug(pl, prob, cur);
        ans += pl*prob;
    ...
    if(par != -1)prob = prob /(graph[cur].size()-1);
    else prob = prob / graph[cur].size();
    for(auto child : graph[cur]) if(child != par) dfs(child, cur, prob, pl+1);
}
