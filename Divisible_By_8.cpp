...
string getNearest(string str){
    int diff = 4;
    string req ;
    int start  = (stoi(str)/10*10);
    if(!start)start++;
    REP(i, start, 1000){
        if(i%8 == 0){
            string curstr = to_string(i);
            while(curstr.size() < 3) curstr = "0" + curstr;
            int cur = 0;
            REP(i, 0, 3) cur += (str[i] != curstr[i]);
            if(cur < diff){diff = cur, req = to_string(i);
                while(req.size() < 3) req = "0" + req;
            }
...
void solve(){
    ...
    n = num.size();
    string reqNum;
    REP(i, 0, 3) reqNum.pb(num[n-3+i]);
    string near = (getNearest(reqNum));
    int l = 0;
    string ans;
    REP(i, 0, n-3) ans.pb(num[i]);
    ans += near;
    while(l < ans.size() && ans[l] == '0') l++;
    // cout << num << ' ';
