...
#define UCHIHA cout << boolalpha << fixed << setprecision(9);
...
void _print(T a, U n){REP(x, 0, n) cerr << a[x] << " \n"[x==n-1];}
template <typename T, typename... V>
...
double f(double m, int n){
    double val = INT_MIN;
    REP(i, 0, n) val = max(val, sqrt((m-x[i])*(m-x[i]) + y[i]*y[i]));
...
bool solve(int n){
    ...
    double l = 2e6+2, r= -2e6-2;
    if(n == 0) return false;
    REP(i, 0, n) cin >> x[i] >> y[i], l = min(l, x[i]), r = max(r, x[i]);
    while(r-l > EPS){
        double m1 = l+((r-l)/3), m2 = r-((r-l)/3);
        double fm1 = f(m1, n), fm2 = f(m2, n);
        if(fm1 > fm2)l = m1;
        else r = m2;
    }
