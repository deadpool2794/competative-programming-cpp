...
void solve(){
    int n; cin >> n;
    int arr[n]; REP(x, 0, n-1) cin >> arr[x];
    vi perm; perm.pb(arr[0]);
    int i = 1, fst = 0, scd = 0;
    while(i < n-1)perm.pb(arr[i]-arr[i-1]), i++;
    sort(ALL(perm));
    ...
    while(i <= n)mp[i++] = 1;
    for(int i : perm){
        if(mp.find(i) == mp.end()) fst += i;
        else if(mp[i] == 1) mp[i]--;
        else fst += i;
    }
    if(fst == 0){
        int cnt = 0;
        for(auto p : mp)if(p.S) cnt++;
        if(cnt == 1) yes();
        else no();
        return;
    }else{
        int scd = 0, cnt = 0;
        for(auto p : mp)if(p.S) scd += p.F, cnt++;
        if(cnt > 2) no();
        else if(scd == fst) yes();
        else no();
    }
