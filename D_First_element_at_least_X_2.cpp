...
struct SegmentTree{
    int sz;
    vector<item> tree;
    item N = node(INT_MIN);// Neutral Element;
    ...
    item find(int x, int lx, int rx, int k, int ind){
        if(ind >= rx) return {-1};
        if(k > tree[x].p) return {-1};
        if(rx-lx == 1) return {lx};
        int m = (lx+rx)/2;
        item res = find(2*x+1, lx, m, k, ind);
        if(res.p == -1) res = find(2*x+2, m, rx, k, ind);
        return res;
    }

    item find(int k, int ind){
        if(k > tree[0].p) return {-1};
        return find(0, 0, sz, k, ind);
    } 
    // void print(){
    //     REP(x, 0, 2*sz-1) cout << tree[x].p << " \n"[x == 2*sz-2];
    // }
};
